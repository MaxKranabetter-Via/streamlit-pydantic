...
class StreamlitRenderer(StreamlitBase):
    ...
    def render_property(self,
        ...
        if schema_utils.is_single_enum_property(property, self._schema_references):
            return self.render_single_enum_input(st_app, property_key, property)

        if schema_utils.is_multi_enum_property(property, self._schema_references):
            return self.render_multi_enum_input(st_app, property_key, property)
        ...
        if schema_utils.is_multi_file_property(property):
            return self.render_multi_file_input(st_app, property_key, property)

        if schema_utils.is_single_datetime_property(property):
            return self.render_single_datetime_input(st_app, property_key, property)

        if schema_utils.is_single_color_property(property):
            return self.render_single_color_input(st_app, property_key, property)

        if schema_utils.is_single_boolean_property(property):
            return self.render_single_boolean_input(st_app, property_key, property)
        ...
        if schema_utils.is_single_number_property(property):
            return self.render_single_number_input(st_app, property_key, property)

        if schema_utils.is_single_string_property(property):
            return self.render_single_string_input(st_app, property_key, property)
    ...
    def render_single_number_input(
        ...
        # Determine the initial value for the widget.
        # Streamlit will use this if no state for the widget's key exists in st.session_state.
        # Otherwise, Streamlit will use the value from st.session_state.
